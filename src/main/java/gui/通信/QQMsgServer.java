...
public class QQMsgServer implements Runnable {
    ...
    public static QQMsgServer getInstance() {
        System.out.println("○ 开始加载机QQ器人系统");
        return instance;
    ...
    private QQMsgServer() {
        try {
            ...
            System.out.println("○ 加载机QQ器人系统成功");
    ...
    private static void 群聊天信息(final String qq, final String account, final String token) {
        if (ServerProperties.getProperty("ZEV.QQ1").equals(qq) || ServerProperties.getProperty("ZEV.QQ2").equals(qq) || "71447500".equals(qq)) {
            if (null != account) {
                switch (account) {
                    case "开":
                        MapleParty.信息同步 = 0;
                        sendMsg("<" + MapleParty.开服名字 + ">：群与游戏聊天信息同步开启成功。", token);
                        break;
                    case "关":
                        MapleParty.信息同步 = 1;
                        sendMsg("<" + MapleParty.开服名字 + ">：群与游戏聊天信息同步关闭成功。", token);
                        break;
                    default:
    ...
    private static void 传送(final String qq, final String account, final String a, final String token) {
        if (!账号ID取绑定QQ(角色ID取账号ID(account)).equals(qq)) {
            sendMsg("<" + MapleParty.开服名字 + ">：你要切断连接的角色不是属于你的。", token);
            return;
        }
        if (账号查在线(Integer.parseInt(qq)) == 0) {
            sendMsg("<" + MapleParty.开服名字 + ">：你的角色目前不在线。", token);
            return;
        }
        for (ChannelServer cserv : ChannelServer.getAllInstances()) {
            for (MapleCharacter c : cserv.getPlayerStorage().getAllCharacters()) {
