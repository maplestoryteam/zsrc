...
public class World {
    ...
    public static void toggleDrops() {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    public static void setPermanentWeather(int i) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    public static void broadcastMessage(byte[] startMapEffect) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
    ...
    public static class Party {
        ...
        public static void partyChat(int partyid, String chattext, String namefrom) {
            ...
            if (party == null) {
                throw new IllegalArgumentException("没有指定的");
            }
...
//                                if (chr.Getcharacterz("" + chr.getId() + "", 4) <= 0) {
//                                    QQMsgServer.sendMsgToQQGroup("[游戏][" + partychar.getName() + "]:  " + chattext);
//                                }
//                            }
//                        }
                    }
    ...
    public static class Buddy {//关于说话文件
    ...
    public static class Alliance {
        ...
        static {
            //System.out.println("");//联 盟 已 加 载 完 成 : : :
    ...
    public static class Family {
        ...
        static {
            //  System.out.println("");//学 院 已 加 载 完 成 : : :
    ...
    public static void registerRespawn() {
        WorldTimer.getInstance().register(new Respawn(), gui.Start.ConfigValuesMap.get("地图刷新频率"));
    }

    public static void registerRespawn2() {
        MapTimer.getInstance().register(new Respawn2(), gui.Start.ConfigValuesMap.get("地图刷新频率"));
    }
    ...
    public static class Respawn2 implements Runnable {
            ...
            public void run() {
                ...
                if (rate.equals("经验")) {
                    for (ChannelServer cservs : ChannelServer.getAllInstances()) {
                        cservs.setExpRate(Integer.parseInt(ServerProperties.getProperty("ZEV.Exp")));
                    ...
                    if (gui.Start.ConfigValuesMap.get("QQ机器人开关") == 0) {
                        sendMsgToQQGroup("<" + MapleParty.开服名字 + ">机器人:经验倍率活动已经结束，已经恢复正常值");
                    }
                } else if (rate.equals("爆率")) {
                    for (ChannelServer cservs : ChannelServer.getAllInstances()) {
                        cservs.setDropRate(Integer.parseInt(ServerProperties.getProperty("ZEV.Drop")));
                        cservs.broadcastPacket(MaplePacketCreator.serverNotice(6, "[系统公告]：爆物倍率活动已经结束，已经恢复正常值。"));
                    }
                    if (gui.Start.ConfigValuesMap.get("QQ机器人开关") == 0) {
                        sendMsgToQQGroup("<" + MapleParty.开服名字 + ">机器人:爆物倍率活动已经结束，已经恢复正常值");
                    }
                } else if (rate.equals("金币")) {
                    for (ChannelServer cservs : ChannelServer.getAllInstances()) {
                        cservs.setMesoRate(Integer.parseInt(ServerProperties.getProperty("ZEV.Meso")));
                        cservs.broadcastPacket(MaplePacketCreator.serverNotice(6, "[系统公告]：金币倍率活动已经结束，已经恢复正常值。"));
                    }
                    if (gui.Start.ConfigValuesMap.get("QQ机器人开关") == 0) {
                        sendMsgToQQGroup("<" + MapleParty.开服名字 + ">机器人:金币倍率活动已经结束，已经恢复正常值");
                    }
                    ...
                    if (gui.Start.ConfigValuesMap.get("QQ机器人开关") == 0) {
                        sendMsgToQQGroup("<" + MapleParty.开服名字 + ">机器人:BOSS爆物倍率活动已经结束，已经恢复正常值");
                    }
...
//                    }
     ...
     * @param hurt
     */
    public static void handleCooldowns2(MapleCharacter chr, int numTimes, boolean hurt) { //is putting it here a good idea? expensive?
        if (chr == null) {
            return;
        }
        ...
        if (chr.isAlive()) {
            if (chr.canRecovery()) {//团队治疗时间
                chr.doRecovery();//团队治疗
            }
//            if (chr.canExpiration(now)) {//期限道具到期消失。
//                chr.expirationTask(true);
//            }
//            /*if (chr.canFish(now)) {//钓鱼
//                chr.doFish(now);
//            }*/
...
//            }
            if ((numTimes % 7 == 0) && (chr.getMount() != null) && (chr.getMount().canTire(now))) {
                ...
                if (chr.getMount().getFatigue() >= 90) {//判断坐骑疲劳值大于等于90
            ...
            if (numTimes % 26 == 0) {
                ...
                for (MaplePet pet : chr.getSummonedPets()) {
                    ...
                    int newFullness = pet.getFullness() - PetDataFactory.getHunger(pet.getPetItemId());
                    if (newFullness <= 5) {//如果宠物饱满度低于或等于5
                        pet.setFullness(15);//给15点饱满度
                        chr.unequipPet(pet, true);//调用召唤宠物的包，把宠物收走
            ...
            if (hurt) {
                if (chr.getInventory(MapleInventoryType.EQUIPPED).findById(chr.getMap().getHPDecProtect()) == null) {
                    ...
                    if ((chr.getMapId() == 749040100 && chr.getInventory(MapleInventoryType.CASH).findById(5451000) == null) || (chr.getMapId() >= 211000000 && chr.getMapId() <= 211999999)) { //minidungeon
                        chr.addHP(-chr.getMap().getHPDec());
                    } else if (chr.getMapId() != 749040100) {
                        chr.addHP(-chr.getMap().getHPDec());
                    }
