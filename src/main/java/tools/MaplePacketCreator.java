...
public class MaplePacketCreator {
    ...
    public static byte[] sendLieDetector(byte[] image, int attempt) {//测谎仪

        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.LIE_DETECTOR.getValue());
        //mplew.write(7);
        mplew.write(0x09);//1 =不攻击，2 =测试，3 =通过
    ...
    public static final byte[] getServerIP(final int port, final int clientId) {
        ...
        mplew.writeShort(0);
        try {
            mplew.write(InetAddress.getByName(MapleParty.IP地址).getAddress());//绑定IPZEV
    ...
    public static byte[] getChannelChange(InetAddress inetAddr, int port) {
        ...
        try {
            ...
            //  mplew.write(addr);
            mplew.write(InetAddress.getByName(MapleParty.IP地址).getAddress());
            //  mplew.write(InetAddress.getByName(服务端IP地址).getAddress());
    ...
    public static byte[] blockedPortal() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
    ...
    public static byte[] weirdStatUpdate() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static final byte[] updateSp(MapleCharacter chr, final boolean itemReaction) {
        if (调试 == "开") {
    ...
    public static final byte[] getWarpToMap(final MapleMap to, final int spawnPoint, final MapleCharacter chr) {//过地图
...
//        mplew.writeInt(skillId);
    ...
    public static byte[] serverNotice2(int type, String message) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] yellowChat(String msg) {//黄色的公告
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
    ...
    public static byte[] serverMessage(String message) {//顶端公告
        if (调试 == "开") {
            System.out.println("【封包】顶端公告");
        }
        return serverMessage(4, 0, message, true);

    }

    private static byte[]  serverMessage(int type, int channel, String message, boolean megaEar) {//喇叭
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
            System.out.println("【封包】文本输出");
        }
        /*
         ...
         */
        // mplew.writeShort(SendPacketOpcode.YELLOW_CHAT.getValue());//YELLOW_CHAT
        mplew.writeShort(SendPacketOpcode.SERVERMESSAGE.getValue());//YELLOW_CHAT
        mplew.write(type);
        if (type == 4) {
            mplew.write(1);
        }
        ...
        switch (type) {
            ...
            case 6:
            case 18:
                mplew.writeInt(channel >= 1000000 && channel < 6000000 ? channel : 0); //现金对应的名称，在黄色显示
                //所有新的出口优惠券，红宝石优惠券，现在可以在现货商店买到！
                //带有红宝石的优惠券是黄色的，带有商品信息
                break;
        }
        return mplew.getPacket();
    }

    public static byte[] getGachaponMega(final String name, final String message, final IItem item, final byte rareness, int channel) {//绿色的喇叭
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] getGachaponMega2(final String message) {//绿色的喇叭
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.SERVERMESSAGE.getValue());
        mplew.write(14);
        mplew.writeMapleAsciiString(message);
    ...
    public static byte[] 红底黄字喇叭2(final String name, final String message, final IItem item, final byte rareness, int channel) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
            System.out.println("getGachaponMega--------------------");
        }
    ...
    public static byte[] 红底黄字喇叭(final String name, final String message, final IItem item, final byte rareness, int channel) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
            System.out.println("getGachaponMega--------------------");
        }
    ...
    *2透明白底蓝字
    ...
    public static final byte[] GainEXP_Monster(final int gain, final boolean white, final int wedding_EXP, final int partyinc, final int Class_Bonus_EXP, final int Equipment_Bonus_EXP, final int Premium_Bonus_EXP) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        if (调试 == "开") {
            System.out.println("【封包】获取经验");
        }
...
//        mplew.writeInt(0);
    ...
    public static byte[] summonAttack(final int cid, final int summonSkillId, final byte animation, final List<SummonAttackEntry> allDamage) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
//
//       if (调试 == "开") {
...
//            mplew.writeLong(0);
    ...
    public static byte[] pet_updateInventorySlot(MapleInventoryType type, IItem item, boolean fromDrop) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//	mplew.write((slot2 > 0 ? 1 : 0) + 1);
    ...
    public static byte[] loveEffect() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
            System.out.println("loveEffect--------------------");
        }
        mplew.writeShort(72);
...
//D6 00 00 00 00 00 01 00 00 00 00 DC DD 40 00 01 00 00 00 01 00 00 00 8A 1C 3D 00 01 00 00 00 00 00 00 00 00 B0 AD 01 00
    ...
    public static byte[] updateCharLook(MapleCharacter chr) {
        ...
        addMarriageRingLook(mplew, chr);
        mplew.writeInt(0);
        //mplew.writeZeroBytes(5); //可能是结婚戒指（1）-> charid跟随（4）*20171202   
        //mplew.writeZeroBytes(1); //可能是结婚戒指（1）-> charid跟随（4）*20171202
        return mplew.getPacket();
    }
...
//        mplew.write(isSelf ? 1 : 0);
    ...
    /* 专门为了疾驰而加的*/
    public static byte[] giveDash(List<Pair<MapleBuffStat, Integer>> statups, int duration) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] showDashEffecttoOthers(int cid, List<Pair<MapleBuffStat, Integer>> statups, int duration) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] givePirateBuff(int buffid, int bufflength, List<Pair<MapleBuffStat, Integer>> statups) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.GIVE_BUFF.getValue());
        //  long mask = getLongMask(statups);
        mplew.writeLong(MapleBuffStat.ENERGY_CHARGE.getValue());
    ...
    public static byte[] 能量条(List<Pair<MapleBuffStat, Integer>> statups, int duration) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        // long mask = getLongMask(statups);
        mplew.writeShort(SendPacketOpcode.GIVE_BUFF.getValue());
        mplew.write(0);
    ...
    public static byte[] giveForeignBuff(MapleCharacter c, int cid, List<Pair<MapleBuffStat, Integer>> statups, MapleStatEffect effect) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] cancelForeignBuffMONSTER(int cid, List<MapleBuffStat> statups) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] cancelBuffMONSTER(List<MapleBuffStat> statups) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//        mplew.writeShort(c.getJob());
    ...
    public static byte[] getTradeInvite(MapleCharacter c, boolean 现金交易) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] getTradeStart(MapleClient c, MapleTrade trade, byte number, boolean 现金交易) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//        mplew.writeShort(c.getPlayer().getJob());
    ...
    public static byte[] 游戏屏幕中间黄色字体(String status) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        if (调试 == "开") {
            System.out.println("【封包】游戏屏幕中间黄色字体");
        }
        mplew.writeShort(SendPacketOpcode.SHOW_STATUS_INFO.getValue());
        mplew.write(1);
    ...
    public static byte[] 游戏屏幕中间黄色字体(String status, int id) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//        mplew.writeInt(skillId);
    ...
    public static byte[] multiChat(String name, String chattext, int mode) {
        //if (server.Start.ConfigValuesMap.get("群显示游戏聊天开关") == 0) {
        /*if (MapleParty.机器人待机 == 0) {
            if (MapleParty.信息同步 == 0) {
                QQMsgServer.sendMsgToQQGroup("[游戏][" + name + "]:  " + chattext);
            }
        }*/
        //}
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//        mplew.write(0); //?
    ...
    public static byte[] showQuestMsg(final String msg) {//月秒
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
    ...
    public static byte[] HSText(String m) {
        if (调试 == "开") {
    ...
    public static byte[] Mulung_Pts(int recv, int total) {
        ...
        return showQuestMsg("[武陵塔信息]:获得 " + recv + " 修炼点数, 目前累计了 " + total + " 点修炼点数");
    }
     ...
     * <临时能力开始>
     */
    public static byte[] addTutorialStats() {
     ...
     * <临时能力结束>
     */
    public static byte[] removeTutorialStats() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] updateBeans(int cid, int beansCount) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.UPDATE_BEANS.getValue());//SendPacketOpcode.UPDATE_BEANS.getValue()
        mplew.writeInt(cid);
        mplew.writeInt(beansCount);
        mplew.writeInt(0);
        return mplew.getPacket();
    ...
    public static byte[] showBeans(int 力度, int size, int Pos, int Type) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
    ...
    public static byte[] showCharCash(MapleCharacter chr) {
        ...
        mplew.writeInt(chr.getId());//显示抵用券数量
    ...
    public static byte[] removeLove(int oid) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
    ...
    public static byte[] licenseRequest() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] showForcedEquip() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
    ...
    public static byte[] spawnTutorialSummon(int type) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (调试 == "开") {
    ...
    public static byte[] testPacket(byte[] testmsg) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.write(testmsg);
        mplew.writeZeroBytes(100000);
        System.out.println("++" + mplew.getPacket());
        return mplew.getPacket();
    }
    ...
    public static byte[] showString(String effect) {
        throw new UnsupportedOperationException("Not supported yet.");
    }

    public static void writeMapleAsciiString(String httpurlcn4E8clJR) {
        throw new UnsupportedOperationException("Not supported yet.");
    }

    public static byte[] 开始豆豆(int Cout, int Intensity, byte x, short S, int 豆豆类型) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(0x15D);
        mplew.write(0);
        mplew.write(Cout);
        mplew.writeShort(Intensity);
        mplew.write(豆豆类型);//0普通珠子,1绿色珠子,2蓝色珠子,3红色珠子,4没有珠子
        mplew.writeInt(5);
        return mplew.getPacket();
    }
    //中奖提示: 0x15B,1中奖提示 0没有显示,1显示"中奖"信息,2提高中奖概率

    public static byte[] 出豆豆特效(int 豆豆数量) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(0x15D);
        mplew.write(8);
        mplew.writeInt(豆豆数量);
        mplew.write(2);//不知道是不是豆豆机类型...
        mplew.write(5);

        return mplew.getPacket();
    }

    public static byte[] 打开奖励NPC() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(0x15D);
        mplew.write(6);
    ...
    public static byte[] 切换模式(int 模式) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(0x15D);
        mplew.write(7);
        mplew.write(模式);
    ...
    public static byte[] 奖励信息(int 道具ID) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(0x15D);
        mplew.write(9);
        mplew.writeInt(道具ID);
    ...
    public static byte[] 走马灯(int 走马灯数量) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(0x15D);
        mplew.write(3);
        mplew.write(走马灯数量);
    ...
    public static byte[] 豆豆机_抽奖(MapleCharacter chr, int[] 行数) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(0x15D);
        mplew.write(4);
        mplew.write(1);
        ...
        mplew.write(chr.是否中奖 ? 1 : 0);//是否开启中奖信息
        if (chr.是否中奖) {
            mplew.write(chr.中奖概率);//中奖概率
            mplew.writeInt(chr.中奖豆豆数);//奖励豆豆数
        }
        mplew.write(0);
        mplew.write(0);
        mplew.write(0);
