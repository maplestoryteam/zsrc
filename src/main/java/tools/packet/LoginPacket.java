...
public class LoginPacket {
    ...
    public static final byte[] getHello(final short mapleVersion, final byte[] sendIv, final byte[] recvIv) {
        ...
        // mplew.writeMapleAsciiString(ServerConstants.检测客户端小版本);
        mplew.write(recvIv);
        mplew.write(sendIv);
    ...
    public static final byte[] getAuthSuccessRequest(final MapleClient client) {
        ...
        mplew.writeMapleAsciiString(client.getAccountName());
        mplew.write(1);
        /*if (gui.Start.ConfigValuesMap.get("登陆公告开关") == 0) {
            client.sendPacket(MaplePacketCreator.serverNotice(1, 本地取特定广播()));
        }*/
        return mplew.getPacket();
    }
