...
public static byte[] warpCS(MapleClient c) {
    ...
    public static byte[] warpCSS(MapleClient c) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        //mplew.write(HexTool.getByteArrayFromHexString("00 00 00 63 00 74 00 6F 00 72 00 32 00 44 00 00 00 00 00 8D 00 11 00 D4 00 0E 06 98 E9 86 07 38 C2 BD 07 00 00 65 00 6C 00 00 00 04 00 03 00 D7 01 0C 06 0E 00 00 00 35 00 31 00 31 00 30 00 30 00 30 00 31 00 00 00 00 00 00 00 04 00 07 00 D3 01 0A 06 10 00 00 00 65 00 62 00 75 00 72 00 73 00 74 00 65 00 72 00 00 00 00 00 82 00 0B 00 2F 00 0A 06 B8 05 49 07 88 D6 B3 07 "));
        int[] itemz = CashItemFactory.getInstance().getBestItems();//热销榜
    ...
    public static byte[] sendBlockedMessage(int type) {
    ...
    public static byte[] show塔罗牌(String name, String otherName, int love, int cardId, int commentId) {
    ...
    public static void addModCashItemInfo(MaplePacketLittleEndianWriter mplew, CashModInfo item) {
        ...
        if ((flags & 0x1) != 0) {
            ...
            // System.out.println("addModCashItemInfo-itemid:"+item.itemid);
        }
        if ((flags & 0x2) != 0) {
            mplew.writeShort(item.count);
            // System.out.println("addModCashItemInfo-count:"+item.count);
        }
        if ((flags & 0x10) != 0) {
            mplew.write(item.priority);
            // System.out.println("addModCashItemInfo-priority:"+item.priority);
        }
        if ((flags & 0x4) != 0) {
            mplew.writeInt(item.discountPrice);
            // System.out.println("addModCashItemInfo-discountPrice:"+item.discountPrice);
        }
        if ((flags & 0x8) != 0) {
            mplew.write(0);
            // System.out.println("addModCashItemInfo-unk_1:"+ (item.unk_1 - 1));
        }
        if ((flags & 0x20) != 0) {
            mplew.writeShort(item.period);
            // System.out.println("addModCashItemInfo-period:"+item.period);
        }
        if ((flags & 0x40) != 0) {
            mplew.writeInt(0);
            //  System.out.println("addModCashItemInfo-0x40");
        }
        if ((flags & 0x80) != 0) {
            mplew.writeInt(item.meso);
            // System.out.println("addModCashItemInfo-meso:"+item.meso);
        }
        if ((flags & 0x100) != 0) {
            mplew.write(0);
            // System.out.println("addModCashItemInfo-unk_2:"+(item.unk_2 - 1));
        }
        if ((flags & 0x200) != 0) {
            mplew.write(item.gender);
            // System.out.println("addModCashItemInfo-gender:"+item.gender);
        }
        if ((flags & 0x400) != 0) {
            mplew.write(item.showUp ? 1 : 0);
            //System.out.println("addModCashItemInfo-showUp:"+item.showUp);
        }
        if ((flags & 0x800) != 0) {
            mplew.write(item.mark);
            // System.out.println("addModCashItemInfo-mark:"+item.mark);
        }
        if ((flags & 0x1000) != 0) {
            mplew.write(0);
            // System.out.println("addModCashItemInfo-unk_3:"+(item.unk_3 - 1));
        }
        if ((flags & 0x2000) != 0) {
            mplew.writeShort(0);
            // System.out.println("addModCashItemInfo-0x2000:");
        }
        if ((flags & 0x4000) != 0) {
            mplew.writeShort(0);
            //  System.out.println("addModCashItemInfo-0x4000:");
        }
        if ((flags & 0x8000) != 0) {
            mplew.writeShort(0);
            // System.out.println("addModCashItemInfo-0x8000:");
        }
        if ((flags & 0x10000) != 0) {
            ...
            //System.out.println("addModCashItemInfo-0x10000:-----------------------\r\n\r\n");
        }
    }
    ...
    public static byte[] enableCSorMTS() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] sendGift(int itemid, int quantity, String receiver) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//======================================MTS===========================================
    public static byte[] startMTS(final MapleCharacter chr, MapleClient c) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (ServerConstants.调试输出封包) {
