...
public class PlayerStats implements Serializable {
    ...
    public transient boolean equippedWelcomeBackRing, equippedFairy, 精灵吊坠, 宠物捡金币, 宠物捡物品, hasVac, hasClone, 装备许可证, Berserk = false, isRecalc = false;
    public transient int equipmentBonusExp, 经验倍率, 爆物倍率, cashMod, 装备许可证等级;
    ...
    public transient int def, element_ice, element_fire, element_light, element_psn,鉴定;
    ...
    public void recalcLocalStats(boolean first_login) {
        ...
        int added_sharpeye_rate = 0, added_sharpeye_dmg = 0, Prob = 0;
        magic = localint_;
        watk = 0;
...
//        mpRestore = 0;
        ...
        for (IItem item : chra.getInventory(MapleInventoryType.EQUIPPED)) {
            ...
            localmaxhp_ += equip.getHp();//生命
            localmaxmp_ += equip.getMp();//魔法
            localdex += equip.getDex();//敏捷
            localint_ += equip.getInt();//运气
            localstr += equip.getStr();//力量
            localluk += equip.getLuk();//智力
            magic += equip.getMatk() + equip.getInt();
            watk += equip.getWatk();
            speed += equip.getSpeed();//移动
            jump += equip.getJump();//跳跃
            //System.err.println("localmaxhp_" + localmaxhp_);
        ...
        for (IItem item : chra.getInventory(MapleInventoryType.CASH)) {
            ...
            if (经验倍率 == 1) {
                switch (item.getItemId()) {
                    //双倍经验值卡（一天权）
                    case 5210000:
                        //chra.dropMessage(5, "[经验卡]:双倍经验值卡（一天权）");
                        //周六, 周日 : 00:00 - 24:00
                        if (星期 == 1 || 星期 == 7) {
                            if (时 >= 0 && 时 <= 24) {
                                经验倍率 = 2;
                    ...
                    //双倍经验值卡（七天权）
                    case 5210001:
                        //chra.dropMessage(5, "[经验卡]:双倍经验值卡（七天权）");
                        //周六, 周日 : 00:00 - 24:00
                        if (星期 == 1 || 星期 == 7) {
                            if (时 >= 0 && 时 <= 24) {
                                经验倍率 = 2;
                    ...
                    //双倍经验值卡（白天一天权）
                    case 5210002:
                        //chra.dropMessage(5, "[经验卡]:双倍经验值卡（白天一天权）");
                        //周六, 周日 : 00:00 - 24:00
                        if (星期 == 1 || 星期 == 7) {
                            if (时 >= 0 && 时 <= 24) {
                                经验倍率 = 2;
                    ...
                    //双倍经验值卡（白天七天权）
                    case 5210003:
                        //chra.dropMessage(5, "[经验卡]:双倍经验值卡（白天七天权）");
                        //周六, 周日 : 00:00 - 24:00
                        if (星期 == 1 || 星期 == 7) {
                            if (时 >= 0 && 时 <= 24) {
                                经验倍率 = 2;
                    ...
                    //双倍经验值卡（晚上一天权）
                    case 5210004:
                        //chra.dropMessage(5, "[经验卡]:双倍经验值卡（晚上一天权）");
                        //周六, 周日 : 00:00 - 24:00
                        if (星期 == 1 || 星期 == 7) {
                            if (时 >= 0 && 时 <= 24) {
                                经验倍率 = 2;
                    ...
                    //双倍经验值卡（晚上七天权）
                    case 5210005:
                        //chra.dropMessage(5, "[经验卡]:双倍经验值卡（晚上七天权）");
                        //周六, 周日 : 00:00 - 24:00
                        if (星期 == 1 || 星期 == 7) {
                            if (时 >= 0 && 时 <= 24) {
                                经验倍率 = 2;
                    ...
                    //三倍经验值卡（三小时）
                    case 5211060:
                        //chra.dropMessage(5, "[经验卡]:三倍经验值卡（三小时）");
                        经验倍率 = 3;
                        break;
                    //双倍经验值卡（三小时）
                    case 5211047:
                        //chra.dropMessage(5, "[经验卡]:双倍经验值卡（三小时）");
                        经验倍率 = 2;
                        break;
                    default:
                        break;
            ...
            if (爆物倍率 == 1) {
                switch (item.getItemId()) {
                    ...
                    case 5360000:
                        爆物倍率 = 2;
                        //chra.dropMessage(5, "[爆率卡]:双倍爆率卡（一天权）");
                        break;
                    //双倍爆率卡（一天权）
                    case 5360015:
                        爆物倍率 = 2;
                    ...
                    //双倍爆率卡（一周权）
                    case 5360016:
                        //chra.dropMessage(5, "[爆率卡]:双倍爆率卡（一周权）");
                        爆物倍率 = 2;
                        break;
                    //双倍爆率卡（三小时）
                    case 5360014:
                        //chra.dropMessage(5, "[爆率卡]:双倍爆率卡（三小时）");
                        爆物倍率 = 2;
                        break;
                    default:
                        break;
            ...
            if (item.getItemId() == 5650000) {
                ...
                装备许可证等级 = 10;
                //chra.dropMessage(5, "[装备许可证]:可穿戴装备等级 + 10");
            } else if (装备许可证等级 == 0 && item.getItemId() == 5590000) {
                装备许可证等级 = 5;
                //chra.dropMessage(5, "[装备许可证]:可穿戴装备等级 + 5");
            }
        }
        ...
        if (mount
            ...
            switch (mount.intValue()) {
                ...
                case 3:
                    ...
                    break;
                default:
                    System.err.println("未驯服的怪物骑马等级，Speedmod = " + speedMod + "");
            }
        }
//        //***1***最大生命值
//        if (chra.Getcharacterz("" + chra.getId() + "", 1) > 0) {
//            int 加成 = getMaxHp() / 100 * chra.Getcharacterz("" + chra.getId() + "", 1);
//            localmaxhp += 加成;
//            chra.dropMessage(5, "生命值加成：" + 加成);
//        }
...
/*if (经验倍率 < 3 && item.getItemId() == 5211060) {//三倍经验卡(2小时)
                经验倍率 = 3;//overwrite                      //
            } else if (经验倍率 == 1 && (item.getItemId() == 5210001 || item.getItemId() == 5210004 || item.getItemId() == 5211047)) {//双倍经验卡
                经验倍率 = 2;
            } else if (经验倍率 == 1 && (item.getItemId() == 5210005 || item.getItemId() == 5210000)) {//双倍经验值卡七天(晚)//双倍经验值卡一天权
                if (day >= 1 && day <= 5) {
                    if (hour >= 18 || hour < 6) {
                        经验倍率 = 2;
                    }
            ...
            } else if (经验倍率 == 1 && (item.getItemId() == 5210003 || item.getItemId() == 5210002)) {//双倍经验值卡七天权(白)//双倍经验值卡一天权(白)
                if (day >= 1 && day <= 5) {
                    if (hour < 18 && hour >= 6) {
                        经验倍率 = 2;
                    }
            ...
            } else if (经验倍率 == 1 && item.getItemId() == 5210006 && (hour >= 22 || hour <= 2)) {//双倍经验值一七天(特价)
                经验倍率 = 2;
            }
            if (爆物倍率 == 1) {
                if (item.getItemId() == 5360014 || item.getItemId() == 5360016 || item.getItemId() == 5360015|| item.getItemId() == 5360000) {//双倍爆率
                    爆物倍率 = 2;
                } /*else if (item.getItemId() == 5360000 && hour >= 0 && hour <= 6) {//双倍爆率卡一天权
                    爆物倍率 = 2;
                }*/
