...
public enum ItemLoader {
    ...
    HIRED_MERCHANTWWWWWWWWWWWWWW("hiredmerchitems", "hiredmerchequipment", 5, "packageid"),
        ...
        while (rs.next()) {
            ...
            if (mit.equals(MapleInventoryType.EQUIP) || mit.equals(MapleInventoryType.EQUIPPED)) {
                ...
                if (!login) {
                    ...
                    //if (table_equip == "inventoryequipment") {
    ...
    public void saveHiredItems(List<Pair<IItem, MapleInventoryType>> items, Integer... id) throws SQLException {
        Connection con = DatabaseConnection.getConnection();
        saveHiredItems(items, con, id);
    }
    
     public void saveHiredItems(List<Pair<IItem, MapleInventoryType>> items, final Connection con, Integer... id) throws SQLException {
        List<Integer> lulz = Arrays.asList(id);
        if (lulz.size() != arg.size()) {
            return;
        }
        ...
        query_2.append("itemid, inventorytype, position, quantity, owner, GM_Log, uniqueid, expiredate, flag, `type`, sender,price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?");
        for (String g : arg) {
            query_2.append(", ?");
        }
        query_2.append(")");
