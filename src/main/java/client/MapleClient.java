...
public class MapleClient implements Serializable {
    ...
    public void setLastCallNpcTime(long currentTimeMillis, String npcIdName) {
        lastCallNpcTime = currentTimeMillis;
        lastCallNpc = npcIdName;
    }

    public Boolean canCallNpc(String npcIdName) {
        if (lastCallNpcTime == 0) {
            return true;
        }

        if (!npcIdName.equals(lastCallNpc)) {
            return true;
        }

        return (System.currentTimeMillis() - lastCallNpcTime) > 500;
    }
    ...
    public boolean 检测IP在线() {
        boolean ret = false;
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps = con.prepareStatement("SELECT COUNT FROM accounts set  SessionIP  WHERE loggedin > 0");
            ...
            System.err.println("检测IP在线错误" + ex);
        }
        return ret;
    }
    ...
    public int login(String login, String pwd, boolean ipMacBanned) {
        ...
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps = con.prepareStatement("SELECT * FROM accounts WHERE name = ?");//双登陆
            ps.setString(1, login);
            ResultSet rs = ps.executeQuery();
...
//                    }
//                    if (loginstate > MapleClient.LOGIN_NOTLOGGEDIN) { // 已经登陆
//                        loggedIn = false;
//                        loginok = 7;
//                        try {
...
//    }
    public String getSessionIPAddress() {
        try {
            if (session.remoteAddress().toString().split(":")[0] != null) {
    ...
    public final String getAccountqq() {
        return accountqq;
    }

    public final void setAccountqq(final String accountqq) {
            ...
            public void run() {
                try {
                    if (getLatency() < 0) {
                        ...
                        System.err.println("[服务端]" + CurrentReadable_Time() + " : 检测到卡号，断开卡号链接 √");
                    }
    ...
    public final void sendPing() {
        ...
        PingTimer.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                System.err.println("[服务端]" + CurrentReadable_Time() + " : 心跳检测(" + getSession().isActive()+ ") (" + getSession() + ") √ " + getLatency());
                try {
                    if (getLatency() < 0) {
                        MapleClient.this.disconnect(true, false);
    ...
    public final void sendPing2() {
        ...
        PingTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                System.err.println("[服务端]" + CurrentReadable_Time() + " : P " + getLatency());
            }
        }, 60000);
    }
    ...
    public static String m() throws UnknownHostException, SocketException {
