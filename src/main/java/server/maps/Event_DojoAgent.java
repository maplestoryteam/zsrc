...
public class Event_DojoAgent {
    //工作人员强化特别训练场
    private final static int baseAgentMapId = 970030000; 
    ...
    // 925023000 ~ 925023009
    // 925023600 ~ 925023609
    public static boolean warpNextMap(final MapleCharacter c, final boolean fromResting) {//武陵塔
        try {
                ...
                if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
                    for (MaplePartyCharacter mem : c.getParty().getMembers()) {
                        ...
                        if (chr != null) {
                            ...
                            BossRankManager.getInstance().setLog(chr.getId(), chr.getName(), "武陵塔可消耗修炼点", (byte) 2, (byte) point);
                            BossRankManager.getInstance().setLog(chr.getId(), chr.getName(), "武陵塔累计的修炼点", (byte) 2, (byte) point);
                        }
                    }
                } else {
                    ...
                    BossRankManager.getInstance().setLog(c.getId(), c.getName(), "武陵塔可消耗修炼点", (byte) 2, (byte) point);
                    BossRankManager.getInstance().setLog(c.getId(), c.getName(), "武陵塔累计的修炼点", (byte) 2, (byte) point);
