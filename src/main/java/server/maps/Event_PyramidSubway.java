...
public class Event_PyramidSubway {
    ...
    public final void commenceTimerNextMap(final MapleCharacter c, final int stage) {
        ...
        final int time = (type == -1 ? 60 : (stage == 1 ? 240 : 300)) - 1;
    ...
    public final void onMiss(final MapleCharacter c) {
        ...
        energybar -= 5;
        broadcastUpdate(c);
        broadcastEnergy(c, "大屠杀的_小姐", miss);
    }
    ...
    public final void succeed(final MapleCharacter c) {
        ...
        if (type == -1) {
            if (tk >= 700) {
                rank = 0;
            } else if (tk >= 1500 && tk <= 699) {
                rank = 1;
            } else if (tk >= 1000 && tk <= 499) {
                rank = 2;
            } else if (tk >= 500 && tk <= 299) {
                rank = 3;
            }
        } else {
             if (tk >= 700) {
...
//        }
        ...
        c.getClient().sendPacket(MaplePacketCreator.showEffect("pvp/victory"));
    ...
    private static final void changeMap(final MapleCharacter c, final MapleMap map, final int minLevel, final int maxLevel, final int clear) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null && chr.getId() != c.getId() && chr.getLevel() >= minLevel && chr.getLevel() <= maxLevel) {
                    if (clear == 1) {
                        chr.getClient().sendPacket(MaplePacketCreator.showEffect("pvp/victory"));
                    } else if (clear == 2) {
                        chr.getClient().sendPacket(MaplePacketCreator.showEffect("pvp/lose"));
                    }
        ...
        if (clear == 1) {
            ...
            c.getClient().sendPacket(MaplePacketCreator.showEffect("pvp/lose"));
        }
