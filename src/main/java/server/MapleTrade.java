...
public class MapleTrade {
    ...
    public final void CompleteTrade() {
        ...
        FileoutputUtil.packetLog("服务端记录信息/交易记录/" + (chr.get()).getName() + "交易信息.txt", Trade);
        exchangeMeso = 0;
    ...
    public final static void completeTrade(final MapleCharacter c) {
        ...
        if (partner.isLocked()) { // Both locked
            ...
            if (lz == 0 && lz2 == 0) {
                ...
                /*if (Boolean.parseBoolean(ServerProperties.getProperty("ZEV.transaction", "true"))) {//交易记录
                    Trade = "[交易记录完成] " + local.getChr().getName() + " 和 " + partner.getChr().getName() + " 交易完成。\r\n\r\n";
                    FileoutputUtil.packetLog("服务端记录信息/玩家交易记录/" + local.getChr().getName() + ".", Trade);
                    FileoutputUtil.packetLog("服务端记录信息/玩家交易记录/" + partner.getChr().getName() + "", Trade);
                }*/
            } else {
    ...
    public static final void invite现金交易(final MapleCharacter c1, final MapleCharacter c2) {
        if (c1.getMapId() == c2.getMapId() || c1.getMapId() != c2.getMapId()) {
            c1.getClient().sendPacket(MaplePacketCreator.serverNotice(1, "交易被禁止。"));
            return;
        }
        int 玩家交易开关 = gui.Start.ConfigValuesMap.get("玩家交易开关");
        if (玩家交易开关 > 0) {
        ...
        int 玩家现金交易开关 = gui.Start.ConfigValuesMap.get("玩家现金交易开关");
        if (玩家现金交易开关 > 0) {
            c1.getClient().sendPacket(MaplePacketCreator.serverNotice(1, "管理员从后台关闭了现金物品交易功能。"));
            return;
        }

        if (c1.getMapId() != c2.getMapId()) {
            c1.getClient().sendPacket(MaplePacketCreator.serverNotice(1, "交易对象不在同一地图。"));
            return;
        }
        if (c1 == null || c1.getTrade() == null) {
            return;
